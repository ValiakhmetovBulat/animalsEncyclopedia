...
const CookieName = "authorization_token"

const APIKeyLength = 32
const APIKeyExpireTime = 24 * time.Hour
...
// PasswordSpecSymbols is the set of allowed special characters for passwords.
const PasswordSpecSymbols = "-#!$@£%^&*()_+|~={}[]:\";'<>?,./`\\"
...
var ErrPasswordTooLong = fmt.Errorf("password must be less than %d characters", MaxPasswordLength)

// ErrPasswordPolicyMismatch is thrown when a password does not satisfy
// character class requirements.
var ErrPasswordPolicyMismatch = errors.New("password must contain at least 1 lowercase, 1 uppercase, 1 digit, and 1 special character")
...
func CheckPasswordPolicy(password string) error {
	var hasLower, hasUpper, hasDigit, hasSpec bool
	for _, r := range password {
		...
		case r >= '0' && r <= '9':
			hasDigit = true
		case strings.ContainsRune(PasswordSpecSymbols, r):
			hasSpec = true
		}
	}
	...
	case !hasLower || !hasUpper || !hasDigit || !hasSpec:
		return ErrPasswordPolicyMismatch
	}
	return nil
}
...
func GetApiKeyFromContext(c *gin.Context) string {
	...
	if ak == "" {
		if header := c.GetHeader("Authorization"); strings.HasPrefix(header, "Bearer ") {
			ak = strings.TrimPrefix(header, "Bearer ")
		}
	}
